...
def webhook(**kwargs):
	...
	# Build media attach URL
	media_path = data.get("media_path") or ""
	attach_url = ""
	if media_path:
		from urllib.parse import quote as _quote
		attach_url = f"/api/method/crm.api.whatsapp.get_media?filename={_quote(media_path)}"

	# --- Publish realtime event FIRST so Chats page updates immediately ---
	frappe.publish_realtime(
		"whatsapp_chat_update",
		{
			...
			"phone": phone_number,
			"message": {
				"name": msg_id or frappe.generate_hash(length=10),
				"type": "Incoming",
				"from": phone_number,
				"to": "",
			...
			"chat_update": {
				...
				"last_message_sender_name": data.get("sender_name", ""),
			},
		},
		after_commit=False,
	)

	# --- Then try to create WhatsApp Message doc for Lead/Deal Activities tab ---
	doc_name = ""
	try:
		...
		frappe.log_error(title="WhatsApp Bridge: Webhook doc creation failed")

	return {"ok": True, "name": doc_name}
